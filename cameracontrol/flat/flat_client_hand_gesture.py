...
def main():
    ...
    try:
        while True:
            ...
            # --- Robust OpenCV Keyboard Controls ---
            key_raw = cv2.waitKey(1)
            key = key_raw & 0xFF  # Strip away modifier bits (NumLock/CapsLock)
            
            if key_raw != -1:  # If ANY key was pressed
                if key == ord("q"):
                    print("[client] 'q' pressed. Exiting...")
                    break
                elif key == ord("1") or key == 49:  # 49 is the direct ASCII code for '1'
                    print("[client] Key '1' registered. Sending STANDBY...")
                    sender.send("standby")
                elif key == ord("2") or key == 50:  # 50 is the direct ASCII code for '2'
                    print("[client] Key '2' registered. Sending RESUME...")
                    sender.send("resume")
    finally:
        cap.release()
